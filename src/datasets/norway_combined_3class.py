...
__all__ = [
    ...
    "read_norway_combined_laz",
]


def read_norway_combined_laz(path: str, rgb: bool = True) -> Data:
    """Read a LAZ tile from any of the four regions. The LAS classification
    field is mapped to train ids via ID2TRAINID."""
    las = laspy.read(path)
    data = Data()
...
class NorwayCombined3ClassALS(BaseDataset):
    """Combined Viken + Oslo + Bergen2020 + Bergen2022 ALS dataset, 3-class
    (ground / not_ground / building). Tile ids are region-prefixed.
    """
    ...
    def num_classes(self) -> int:
        return NORWAY_COMBINED_3CLASS_NUM_CLASSES
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> Data:
        return read_norway_combined_laz(raw_cloud_path, rgb=self.rgb)
...
class MiniNorwayCombined3ClassALS(NorwayCombined3ClassALS):
    """Mini variant: a small subset (5/2/2 per region) for quick development
    runs. Tile membership is fixed and comes from norway_combined_tiles.py."""
