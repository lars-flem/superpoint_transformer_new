...
from src.datasets.bro_config import (
    BRO_NUM_CLASSES,
    CLASS_COLORS,
    CLASS_NAMES,
    ID2TRAINID,
...
__all__ = ["BroALS", "MiniBroALS"]


def read_bro_las(path: str, rgb: bool = False) -> Data:
    las = laspy.read(path)
    data = Data()
    ...
    data.y = torch.from_numpy(ID2TRAINID[cls]).long()
    ...
    if rgb:
        try:
            ...
            if has_rgb:
                ...
                data.rgb = torch.from_numpy(rgb_data / 65535.0).clamp(0, 1)
            else:
...
class BroALS(BaseDataset):
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> Data:
        return read_bro_las(raw_cloud_path, rgb=self.rgb)
...
"""
    ...
    def id_to_relative_raw_path(self, cloud_id: str) -> str:
        ...
        return osp.join(stage, base_id + ".las")
    ...
    def processed_to_raw_path(self, processed_path: str) -> str:
        ...
        return osp.join(self.raw_dir, raw_split, base_cloud_id + ".las")


class MiniBroALS(BroALS):
    _NUM_MINI = 2
