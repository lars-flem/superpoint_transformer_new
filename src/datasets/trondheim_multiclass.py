...
from src.datasets.trondheim_multiclass_config import (
    CLASS_COLORS,
    CLASS_NAMES,
    ID2TRAINID,
    TILES,
    TRONDHEIM_MULTICLASS_NUM_CLASSES,
)
...
__all__ = ["TrondheimMulticlassALS", "MiniTrondheimMulticlassALS", "read_trondheim_multiclass_laz"]


def read_trondheim_multiclass_laz(path: str, rgb: bool = True) -> Data:
    las = laspy.read(path)
    data = Data()
...
class TrondheimMulticlassALS(BaseDataset):
    ...
    def num_classes(self) -> int:
        return TRONDHEIM_MULTICLASS_NUM_CLASSES
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> Data:
        return read_trondheim_multiclass_laz(raw_cloud_path, rgb=self.rgb)
...
class MiniTrondheimMulticlassALS(TrondheimMulticlassALS):
    _NUM_MINI = 2
