...
from src.datasets.gjerdrum_config import (
    CLASS_COLORS,
    CLASS_NAMES,
    ID2TRAINID,
    TILES,
    GJERDRUM_NUM_CLASSES,
)
...
__all__ = ["GjerdrumALS", "MiniGjerdrumALS"]


def read_gjerdrum_laz(path: str, rgb: bool = False) -> Data:
    las = laspy.read(path)
    data = Data()
...
class GjerdrumALS(BaseDataset):
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> Data:
        return read_gjerdrum_laz(raw_cloud_path, rgb=self.rgb)
...
class MiniGjerdrumALS(GjerdrumALS):
    _NUM_MINI = 2
