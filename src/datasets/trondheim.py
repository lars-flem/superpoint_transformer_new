...
from src.datasets.trondheim_config import (
    CLASS_COLORS,
    CLASS_NAMES,
    ID2TRAINID,
    TILES,
    TRONDHEIM_NUM_CLASSES,
)
...
__all__ = ["TrondheimALS", "MiniTrondheimALS", "read_trondheim_laz"]


def read_trondheim_laz(path: str, rgb: bool = True) -> Data:
    las = laspy.read(path)
    data = Data()
...
class TrondheimALS(BaseDataset):
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> Data:
        return read_trondheim_laz(raw_cloud_path, rgb=self.rgb)
...
class MiniTrondheimALS(TrondheimALS):
    _NUM_MINI = 2
